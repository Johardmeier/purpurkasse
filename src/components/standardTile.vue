<template>
  <v-layout row justify-start class="ticketPanel" :class="borderClass">

    <div class="sidePanel">
      <div>Reserviert</div>
      <div>{{openBookings}}</div>
      <div>{{ticketsCount + tarifsCount + keepOpen - openBookings | numberDiff }}</div>
    </div>

    <v-flex xs6 >
      <v-layout justify-start align-center fill-height mx-4>
  ...
  export default {
    ...
    computed: {
      borderClass: function (){
        return(this.isAdult ? 'adultBorder' : 'childBorder')
      },
      tarifsSum: function() {
        let tSum=0
